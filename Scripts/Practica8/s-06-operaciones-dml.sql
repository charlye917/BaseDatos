...
begin
	...
	delete from proyecto_2_1;
	delete from proyecto_2_2;	
	delete from empleado_1;
	delete from empleado_2_1;
	delete from empleado_2_2;
...
begin
	--pais
	insert into v_pais(pais_id,clave,nombre,region) values (1,'MX','MEXICO','A');
	insert into v_pais(pais_id,clave,nombre,region) values (2,'JAP','JAPON','B');
	--oficina
	insert into v_oficina(oficina_id,nombre,pais_id) values(1,'OFICINA 1 DE CDMX',1);
	insert into v_oficina(oficina_id,nombre,pais_id) values(2,'OFICINA 1 DE TOKIO',2);

	--empleado
	insert into v_empleado(empleado_id,nombre,ap_paterno,ap_materno,rfc,email,jefe_id,num_cuenta,foto) 
...
select  
	...
	(select count(*) from empleado_2_1) as emp_2,
	(select count(*) from empleado_2_2) as emp_3
from  dual;
...
begin
	...
	select count(*) into v_count_2 from empleado_2_1;
	select count(*) into v_count_3 from empleado_2_2;
	dbms_output.put_line('C. Sin cambio de sitio - Prueba 1:'||v_count_1||','||v_count_2||','||v_count_3);

	update v_empleado set rfc = 'ZZ'||substr(rfc,3,length(rfc));
	select count(*) into v_count_1 from empleado_1;
	select count(*) into v_count_2 from empleado_2_1;
	select count(*) into v_count_3 from empleado_2_2;
	dbms_output.put_line('D. Cambio de sitio  -    prueba 2:'||v_count_1||','||v_count_2||','||v_count_3);

	update v_empleado set rfc = 'AA'||substr(rfc,3,length(rfc));
	select count(*) into v_count_1 from empleado_1;
	select count(*) into v_count_2 from empleado_2_1;
	select count(*) into v_count_3 from empleado_2_2;
