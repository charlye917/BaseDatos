...
begin
	...
	case
		when INSERTING then
			IF SUBSTR(V_RFC,1,1) BETWEEN 'A' AND 'M' THEN
			...
			ELSIF SUBSTR(V_RFC,1,1) BETWEEN 'N' AND 'Z' THEN
			...
			ELSE
				raise_application_error(20001,
					...
					|| V_RFC);
			END IF;
			--INSERTANDO EL BINARIO, USO DE UNA TABLA TEMPORAL
			INSERT INTO T_EMPLEADO_INSERT(EMPLEADO_ID, FOTO, NUM_CUENTA)
			VALUES(V_EMPLEADO_ID, V_FOTO, V_NUM_CUENTA);
			--INSERTA EN EL SITIO REMOTO A TRAVES DE LA TABLA TEMPORAL
			INSERT INTO EMPLEADO_2_2
		...
		WHEN UPDATING THEN
			...
			IF (SUBSTR(V_RFC,1,1) BETWEEN 'A' AND 'M') AND (SUBSTR(:OLD.RFC,1,1) BETWEEN 'A' AND 'M') THEN
			...
			ELSIF (SUBSTR(V_RFC,1,1) BETWEEN 'N' AND 'Z') AND (SUBSTR(:OLD.RFC,1,1) BETWEEN 'N' AND 'Z') THEN
			...
			INSERT INTO T_EMPLEADO_UPDATE (EMPLEADO_ID, FOTO, NUM_CUENTA)
			VALUES(V_EMPLEADO_ID, V_FOTO, V_NUM_CUENTA);
			UPDATE EMPLEADO_2_2
			SET EMPLEADO_ID = V_EMPLEADO_ID, FOTO =(
				SELECT FOTO
				FROM T_EMPLEADO_UPDATE
				WHERE EMPLEADO_ID = V_EMPLEADO_ID
			) WHERE EMPLEADO_ID = :OLD.EMPLEADO_ID;
			DELETE FROM T_EMPLEADO_UPDATE WHERE EMPLEADO_ID = V_EMPLEADO_ID; 
		end case;
	end;
/
